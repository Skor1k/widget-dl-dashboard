export interface ComponentMapping {
  ...
  defaultFalse?: string[];
  /** Если true — все BOOLEAN пропы компонента, не заданные явно в descriptor, автоматически выключаются.
   * Заменяет ручное перечисление в defaultFalse для компонентов с многими необязательными слотами. */
  autoDisableUnsetBooleans?: boolean;
  /** Если true — AST-парсер обходит компонент как прозрачный контейнер и парсит его детей */
  transparent?: boolean;
  /** Маппинг code prop → имя вложенного INSTANCE в Figma.
   * Для компонентов без TEXT компонент-пропов (AlertLine и т.п.):
   * находим инстанс с указанным именем и пишем текст напрямую в его TEXT-узел. */
  textProps?: Record<string, string>;
  /** Маппинг code prop → имя TEXT-узла внутри компонента (без INSTANCE-обёртки).
   * Для компонентов без exposed TEXT props, где текст лежит в прямом TEXT-узле по имени. */
  textNodeProps?: Record<string, string>;
  /** Маппинг одного React-пропа (обычно color) на набор Figma VARIANT-пропов с true/false.
   * Например: color="accent" → Accent="true". Остальные boolean остаются "false" (дефолт Figma). */
  colorMap?: {
    prop: string;
    map: Record<string, string>; // React value → Figma prop base name
  };
}
...
export const COMPONENT_MAP: Record<string, ComponentMapping> = {
  ...
  FileUploader: {
    ...
    propAliases: {
      isLoading: "State",  // boolean isLoading → VARIANT State (Loading/Normal)
      // isCompact → IsCompact: normalize совпадает, alias не нужен
    },
    valueAliases: {
      isLoading: {
        "true": "Loading",
  ...
  InfoItem: {
    ...
    propAliases: {
      hasImage: "Type",    // boolean → VARIANT Type (Icon/Image)
      onDelete: "Close",   // callback → BOOLEAN Close
      // addonLeft → AddonLeft: normalize совпадает ✓
      // addonRight → AddonRight: normalize совпадает ✓
      // caption → Caption: normalize совпадает ✓
    },
    valueAliases: {
      hasImage: {
        "true": "Image",
    ...
    implicitBooleans: {
      "AddonLeft": ["hasImage"],   // hasImage требует AddonLeft=true
      "AddonRight": ["onDelete"],  // onDelete требует AddonRight=true
    },
  },
  ...
  Label: {
    ...
    propAliases: {
      addonLeft: "IconLeft",    // ReactNode → BOOLEAN IconLeft
      addonRight: "IconRight",  // ReactNode → BOOLEAN IconRight
      // color, size, isDisabled, withIconOnly — normalize совпадает ✓
    },
    valueAliases: {
      color: {
        "black": "inverted",
        "black-negative": "inverted-negative",
        // info, success, error, warning, blue-negative, red-negative,
        // orange-negative, green-negative, message, ai, logo-blue,
        // purple, purple-negative, inverted — совпадают as-is
      },
    },
    // size: prefix-match '4xs'→'4xs (16)', '3xs'→'3xs (20)', '2xs'→'2xs (24)' ✓
  },
  ...
  Cell: {
    ...
    propAliases: {
      isHighlighted: "Highlight",   // isHighlighted → Highlight (BOOLEAN)
      addonBottom: "Caption",       // ReactNode addonBottom → BOOLEAN Caption (вторая строка)
      iconRight: "Details",         // ReactNode iconRight → BOOLEAN Details (иконка справа)
      statusColor: "Status",        // complex type → BOOLEAN Status
      isGrowable: "HugContent",     // boolean → VARIANT HugContent (inverse)
      // size, type, align, rightPadding, leftPadding, color — normalize совпадает, alias не нужен
    },
    valueAliases: {
      color: {
        "success-low-contrast": "SuccessLow",
        "error-low-contrast": "ErrorLow",
        // neutral/success/error → capitalize ✓
      },
      isGrowable: {
        "true": "False",   // isGrowable=true (растягивается на контейнер) → HugContent=False
        "false": "True",   // isGrowable=false (сжимается по контенту) → HugContent=True
      },
    },
  },
  ...
  ElementHeader: {
    ...
    propAliases: {
      textAddonRight: "AddonRight",  // ReactNode → BOOLEAN AddonRight
      badge: "Label",                // ReactNode badge → BOOLEAN Label
      // isBold → IsBold: normalize совпадает, True/False VARIANT fallback ✓
      // horizontalLayout → 💠 HorizontalLayout: normalize совпадает ✓
      // help → Help: normalize совпадает ✓
      // size → Size: prefix-match 'm'→'m (16-24)', 's'→'s (16-20)', 'xs'→'xs (14-20)' ✓
    },
  },
  ...
  Badge: {
    componentKey: "5b672a76f01ffeed3d2e22bf9cffef1c4b21268f",
    propAliases: {
      isDisabled: "Disabled",  // React isDisabled → Figma Disabled (VARIANT true/false)
      // size: normalize совпадает ('8','12','16','20','24'), alias не нужен
    },
    colorMap: {
      prop: "color",
      map: {
        ...
        white: "Contrast",
        black: "Inverted",
        // gray, blue, green, yellow, red — нет Figma-аналогов
      },
    },
    // Dot и digits обрабатываются в code.ts по длине descriptor.text
  },
  ...
  AlertLineSlide: {
    ...
    // title → текст внутри инстанса "#Content" (нет Figma компонент-пропа для Title)
    textProps: { title: "#Content" },
  },

  MediaItem: {
    componentKey: "c61ea000d347e935fa9b1d90ec9820f2038bbe19",
    propAliases: {
      title: "Header",       // ReactNode title → BOOLEAN Header
      controls: "Icons",     // ReactNode controls → BOOLEAN Icons
      isLoading: "Stub",     // boolean → VARIANT Stub (false/true)
      isHoverable: "Hover",  // boolean → BOOLEAN Hover
      // caption → Caption: normalize совпадает ✓
      // Corners, BGWhite, Ratio → нет React-аналогов
    },
  },

  MediaItemCard: {
    componentKey: "7398e96d96a710cd877127bb219bb4107937df1c",
    propAliases: {
      title: "Caption",      // string title → BOOLEAN Caption (основной текст)
      caption: "Price",      // string caption → BOOLEAN Price
      controls: "Icons",     // ReactNode controls → BOOLEAN Icons
      isLoading: "Stub",     // boolean → VARIANT Stub
      isHoverable: "Hover",  // boolean → BOOLEAN Hover
      // ratio → Ratio: normalize совпадает, values as-is ✓
    },
  },

  FloatingButton: {
    componentKey: "d85a3b6397cd2933898b79b93e080d402c24171d",
    // color → Color: capitalize (accent→Accent, contrast→Contrast) ✓
    // size → Size: prefix-match (l→"l (56)", m→"m (44)", ...) ✓
    // isLoading → isLoading: normalize совпадает, VARIANT true/false ✓
    // icon → INSTANCE_SWAP обрабатывается автоматически
  },

  List: {
    componentKey: "0b44251b5340f247ee0535708a61cbca10cd5c6e",
    // size → Size: 'm'/'s'/'xs' as-is ✓
    // width → width: 'auto'/'max' as-is ✓
  },

  ListItem: {
    componentKey: "97404574334366806493fdd4472ef39e3a55aa80",
    propAliases: {
      target: "NewWindow",   // string target → BOOLEAN NewWindow
      // addonLeft → AddonLeft, addonRight → AddonRight,
      // caption → Caption, help → Help, label → Label,
      // isDisabled, size → Size (prefix-match M(44)/S(36)/XS(32)) — auto ✓
    },
  },

  // ListSection → тот же ListItem_V4, но с Type="Header Gray/Black"
  // @deprecated — используйте List + ListItem
  ListBox: {
    componentKey: "162041229ee1509e08ecd83f32831f07f1088c17",
    // Loading → isLoading: normalize совпадает (VARIANT false/true) ✓
    // Search → Search: normalize совпадает (BOOLEAN) ✓
    // Size: prefix-match XS/S/M ✓
  },

  ListSection: {
    componentKey: "97404574334366806493fdd4472ef39e3a55aa80",
    propAliases: {
      color: "Type",   // 'gray'|'black' → Type VARIANT (Header Gray/Header Black)
      // caption → Caption: normalize совпадает ✓
    },
    valueAliases: {
      color: {
  ...
  // Dots: Dots VARIANT (кол-во точек) = items.length — требует спецобработки в code.ts.
  // Color, Size — нет React-аналогов.
  Dots: {
    componentKey: "2fbf62e875c4c3f484560570bb8897488fedaee0",
  },

  // DataItemGroup: слоты 1-7 (BOOLEAN) соответствуют дочерним DataItem.
  // Управление слотами по кол-ву children требует отдельной логики в code.ts.
  DataItemGroup: {
    componentKey: "e8c398c404644896874235ceb3847bc1085dba68",
  },

  DataItem: {
    componentKey: "925dd9a41ab14603c6bf741e27bc6be8e9a56dcc",
    propAliases: {
      caption: "Data",   // ReactNode caption → BOOLEAN Data
      delta: "Type",     // ReactElement delta → VARIANT Type
    },
    valueAliases: {
      delta: {
        "true": "DeltaPositiveNumber",  // delta передан → Type="DeltaPositiveNumber"
      },
    },
    // size → Size: toUpperCase (xs→XS, l→L) ✓
    // help → Help: normalize совпадает ✓
    // Stab → нет React-аналога (skeleton/stub), пропускаем
  },

  CheckButton: {
    componentKey: "3d45d52d529a9ecdc3d6f866a8f7e7a3fd82acee",
    propAliases: {
      isSelected: "Type",  // boolean → VARIANT On/Off
    },
    valueAliases: {
      isSelected: {
        "true": "On",
    ...
    // size: prefix-match → "m (44)"/"s (36)"/"xs (32)" ✓
    // width, isDisabled, color, caption, help — auto ✓
  },
  MicroNotification: {
    componentKey: "b5d5e51e43dfedee35f168943d5f439fcdb8be57",
    propAliases: {
      onClose: "Close",   // callback → BOOLEAN Close
      // caption → Caption: normalize совпадает ✓
      // width → Width: auto/max ✓
    },
  },
  Spinner: {
    componentKey: "54c4f4618cf6f84a7e1c739c8e6bfc00c6ac1086",
    // size → Size: prefix-match с числовыми вариантами ✓
    // color: нет аналога в Figma, игнорируется
  },
  Modal: {
    componentKey: "d1d97a27256b6002e41d1e7c377f001a35cc2623",
    // 🅿🟠 Modal_V4 — в процессе обновления, есть отличия от сторибука
    propAliases: {
      width: "WidthMax",
    },
    valueAliases: {
      // width='max' → Fill, по умолчанию Fixed
      width: { max: "Fill" },
    },
  },

  OverlayContent: {
    componentKey: "f2eff6d10807ebad5ae4aeeb0ad8e5e65e4d903b",
    // ⛔️ OverlayContent — внутренняя часть Modal в Figma
    // width / rightBarWidth — только React, аналогов в Figma нет
    // Menu / State — Figma-specific, не маппятся
    propAliases: {
      rightBar: "RightSidebar",
      leftBar: "LeftSidebar",
      bottomButtons: "Buttons",
    },
    // Figma дефолты True, но React передаёт только по необходимости
    defaultFalse: ["RightSidebar", "LeftSidebar", "Buttons", "Errors"],
  },

  ActionBar: {
    componentKey: "854bcd20c6533d65138f1276d3e250c1b2ea4e04",
    // одиночный компонент, нет VARIANT-пропов
    // menu / quickActions / mandatoryActions — слоты ReactNode, обрабатываются автоматически
    // State → INSTANCE_SWAP внутренний, не маппится
    // AccentButton включается когда передан mandatoryActions
    implicitBooleans: { AccentButton: ["mandatoryActions"] },
    // Close: нет аналога в React ActionBarProps — пропущен
  },
  // ActionBarAction — action-кнопки внутри ActionBar.
  // В Figma ActionBar_V2 они реализованы как Button_V5 в "regular buttons" фрейме.
  ActionBarAction: {
    componentKey: "a765d5679f10f13e5e369e4df617396acca6d397", // Button_V5
  },

  // Прозрачные layout-контейнеры — AST-парсер обходит их детей напрямую
  Island: { transparent: true } as ComponentMapping,
  HStack: { transparent: true } as ComponentMapping,
  VStack: { transparent: true } as ComponentMapping,

  // Компоненты DikidiPage
  ClickableIcon: {
    componentKey: "4c09fc40fc8e320bffd54b85dbd51dc4642e4092",
  },
  Paragraph: {
    componentKey: "43e792c10cbada5d5381285d0ef1c21ffcaf2c36",
  },
  Image: {
    componentKey: "12d5c6a9ff311e073a36d2a28d6157f2a97dd8d5",
  },
  Menu: {
    componentKey: "c8cf62fd8a01eafd5bc624c13e3d656e7f07c522",
  },
};
...
export function mapPropsToFigma(
  ...
  // colorMap: один React-проп (обычно color) → отдельные Figma VARIANT-пропы true/false
  // Например: color="accent" → устанавливает Accent="true"
  if (mapping?.colorMap) {
    const { prop, map } = mapping.colorMap;
    const codeVal = codeProps[prop];
    if (typeof codeVal === "string") {
      const targetPropBase = map[codeVal];
      if (targetPropBase) {
        const targetPropKey = findPropKey(targetPropBase);
        if (targetPropKey) {
          // Figma использует VARIANT "true"/"false", а не BOOLEAN
          figmaProps[targetPropKey] = "true";
        } else {
          warnings.push(`${componentName}.${prop}: Figma-проп "${targetPropBase}" не найден в propDefs`);
        }
      } else {
        warnings.push(`${componentName}.${prop}: "${codeVal}" не найдено в colorMap`);
      }
    }
  }

  for (const [codeProp, codeValue] of Object.entries(codeProps)) {
    // Пропускаем проп, уже обработанный через colorMap
    if (mapping?.colorMap && codeProp === mapping.colorMap.prop) continue;
